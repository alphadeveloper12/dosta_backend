...
def populate():
    print("Starting International Menu (150 AED) Population...")

    # 1. Setup Cuisine
    cuisine, created = Cuisine.objects.get_or_create(name="International")
    # ...

    # 2. Setup Budget Option (150 AED) - Likely Royal based on price
    budget = BudgetOption.objects.filter(price_range="150 AED").first()
    if not budget:
    ...
    menu_structure = {
        "Salads": [
            "SEAFOOD SALAD", # Corrected typo SEAFOOS
            "THAI BEEF PAPAYA",
            "HUMMUS / BABAGANOOG",
            "CARROTS COLESLAW",
        ...
        "Under The Light": [
            "ROASTED LAMB LEGS / LAMB JUS"
        ],
        "Live Stations": [
            "RAMEN",
            "EGG NOODLE, SOBA NOODLE, UDON NOODLE, MUSHROOM, PAK CHOY, CHILI PASTE"
        ],
        "Main Course": [
            "STIR FRIED BEEF WITH BROCCOLI IN OYSTER SAUCE",
            "WASABI MUSTARD PRAWN",
            "KUNG POA CHICKEN",
            "CHICKEN EGG CHOW MIN",
            "JASMINE RICE",
            "MALAYSIAN LAMB CURRY",
            "CHILLI FISH",
            "TERIYAKI GLAZED PAK CHOY",
            "ROASTED SPRING BABY CORN"
        ],
        "Dessert": [
            "TIRAMISU CAKE",
            "MOHALABIEH",
    ...
    for course_name, item_names in menu_structure.items():
        ...
        for raw_name in item_names:
            ...
            # NEW: Image sourcing
            source_folder = os.path.join(django.conf.settings.MEDIA_ROOT, 'source_images', 'International', '150 AED')
    ...
    menu_name = "International Royal Menu"
