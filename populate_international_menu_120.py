...
def populate():
    print("Starting International Menu (120 AED) Population [VERBOSE]...")

    # 1. Setup Cuisine
    cuisine, created = Cuisine.objects.get_or_create(name="International")
    print(f"Cuisine: {cuisine.name} (ID: {cuisine.id})")
    ...
    if not budget:
        ...
        print(f"Created BudgetOption: {budget} (ID: {budget.id})")
    else:
        print(f"Found BudgetOption: {budget} (ID: {budget.id})")
    ...
    menu_structure = {
        ...
        "Main Course": [
            "BAKED SALMON, SALSA VERDE AND CARROT GINGER PUREE",
            "BEEF STROGANOFF",
            "CHICKEN COQ- AU –VIN",
            "LAMB MINCE PIE",
            "JASMINE RICE",
    ...
    for course_name, item_names in menu_structure.items():
        ...
        for raw_name in item_names:
            ...
            # NEW: Image sourcing
            source_folder = os.path.join(django.conf.settings.MEDIA_ROOT, 'source_images', 'International', '120 AED')
    ...
    menu_name = "International Premium Menu"
    ...
    print(f"Population Complete! Menu ID: {fixed_menu.id}")
    print(f"Linked {fixed_menu.items.count()} items.")
